            ...
            <form action="{{ route('sliderUpdate', $slider->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="card-body">
                  ...
                  <div class="row">
                    ...
                    <div class="col-sm-9">
                      <input type="text" name="title" value="{{ $slider->title }}" placeholder="Enter a title" id="title"
                        class="form-control">
                      @error('title')
                    ...
                    <div class="col-sm-9">
                      <select name="category_id" id="category_id" class="form-control">
                        ...
                        @foreach($category as $item)
                          <option @if($slider->category_id == $item->id) selected @endif value="{{ $item->id }}">{{ $item->name }}</option>
                        @endforeach
                      </select>
                      @error('category_id')
                      <small class="text-danger">{{ $message }}</small>
