    ...
    <div class="row">
              ...
              <table id="example" class="display table" style="width: 100%; cellspacing: 0;">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      @if($brandItem->logo)
                      <img src="{{ asset('/backend/uploads/' . $brandItem->logo) }}" class="small-image"
                        alt="Category icon - {{ $brandItem->logo }}">
                      @else
                    ...
                    <td>
                      <span
                        class="badge badge-pill badge-{{ $brandItem->status === 'Active' ? 'success' : 'danger' }}">{{
                        $brandItem->status }}</span>
                    </td>
                    <td>
                      <a class="btn btn-sm btn-info"
                        href="{{ url(routePrefix().'/brand/'. $brandItem->id).'/edit' }}"><i class="fa fa-edit"></i></a>

                      <a class="btn btn-sm btn-info" href="{{ url( routePrefix() . '/brand/' . $brandItem->id) }}"><i
                          class="fa fa-eye"></i></a>

                      <form style="display: inline-block" action="{{ route('brand.destroy', $brandItem->id) }}"
                        method="post">
                        @csrf
                        @method('DELETE')
      ...
      <div class="col-md-6 col-md-offset-3">
        <div class="panel panel-white">
          ...
          <div class="panel-body">
            <form
              action="{{ $page == 'create' ? url(routePrefix() . '/brand') : url(routePrefix() . '/brand/' . $data->id) }}"
              ...
              <div class="form-row">
                <div class="form-group">
                  ...
                  <input type="text" class="form-control m-t-xxs" id="brand" placeholder="brand name" name="name"
                    value="{{ $data ? $data->name : old('name')}}" onkeyup="pushSlug(this.value, 'brand_slug')">
              ...
              <div class="form-row">
                <div class="form-group">
                  ...
                  <input type="file" class="form-control" name="logo" id="brand_logo"
                    onchange="previewImage('logo', this.files)">
                  @error('logo')
                  <small class="text-danger">{{ $message }}</small>
                  @enderror
                </div>
              ...
              <div class="form-row">
                <div class="form-group">
                  ...
                  <input type="text" value="{{ $data ? $data->slug : old('slug') }}" name="slug"
                    class="form-control m-t-xxs" id="brand_slug" placeholder="Slug">
                  @error('slug')
                  <small class="text-danger">{{ $message }}</small>
                  @enderror
                </div>
